...
def find_leaf_nodes(nodes: list[dict], edges: list[dict]) -> list[dict]:
    # Create a set of node IDs that have outgoing edges
    nodes_with_outgoing = {edge["source"] for edge in edges}

    # Filter nodes that are not in the set of nodes with outgoing edges
    leaf_nodes = [node for node in nodes if node["id"] not in nodes_with_outgoing]
