...
def coin_change(coins: list[int], amount: int, index: int) -> int:
    memo = {}
    return coin_change_helper(coins, amount, index, memo)
...
def coin_change_helper(coins: list[int], amount: int, index: int, memo: dict) -> int:
    # Base cases
    if amount == 0:
        return 1
    ...
    memo[(amount, index)] = coin_change_helper(
        coins, amount - coins[index], index, memo
    ) + coin_change_helper(coins, amount, index + 1, memo)
